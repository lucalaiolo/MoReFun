...
def _self_device(module):
    """Best-effort lookup of the device a module's parameters live on."""
    try:
...
class AuxFormer(nn.Module):
    def __init__(self, 
        ...
        # self.agent_embed = nn.Parameter(torch.empty(1, n_agent, 1, h_dim)).to(_self_device(self))
    ...
    def forward(self,all_traj):
        ...
        batch_ind = torch.arange(B)[:,None,None].to(_self_device(self))
        agent_ind = torch.arange(N)[None,:,None].to(_self_device(self))
        ...
        if self.regular_masking:
            ...
            shuffle_indices = torch.rand((B,N,self.past_timestep)).argsort().to(_self_device(self))
            ...
            batch_ind2 = torch.arange(B)[:,None].to(_self_device(self))
        ...
        if self.part_noise:
            noise_mask = torch.rand(B,N,T).to(_self_device(self))
        ...
        past_future_indices = torch.arange(T)[None].repeat(B,1).to(_self_device(self))
    ...
    def mask_forward(self,all_trajs,mask,all_out=False):
        ...
        mask_ind = torch.arange(T)[None,None,:].repeat(B,N,1).to(_self_device(self))
...
class TrajTrans_spatial(nn.Module):
    def __init__(
        ...
        # Add 1 for cls_token
        self.pos_embed = nn.Parameter(torch.randn(1, num_patches, h_dim))
